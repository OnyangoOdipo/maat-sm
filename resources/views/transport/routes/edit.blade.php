<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('transport.routes.update', $route) }}" class="space-y-6" x-data="routeForm({{ json_encode($route->stops) }})">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="name" name="name" type="text" 
                                    class="mt-1 block w-full" :value="old('name', $route->name)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="fee_amount" name="fee_amount" type="number" step="0.01"
                                    class="mt-1 block w-full" :value="old('fee_amount', $route->fee_amount)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="morning_pickup_time" name="morning_pickup_time" type="time" 
                                    class="mt-1 block w-full" :value="old('morning_pickup_time', \Carbon\Carbon::parse($route->morning_pickup_time)->format('H:i'))" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="evening_departure_time" name="evening_departure_time" type="time" 
                                    class="mt-1 block w-full" :value="old('evening_departure_time', \Carbon\Carbon::parse($route->evening_departure_time)->format('H:i'))" required />
                                ...
                                <select id="vehicle_id" name="vehicle_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    @foreach($vehicles as $vehicle)
                                        <option value="{{ $vehicle->id }}" 
                                            {{ old('vehicle_id', optional($route->assignments->where('status', 'active')->first())->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
                                ...
                                <select id="driver_id" name="driver_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    @foreach($drivers as $driver)
                                        <option value="{{ $driver->id }}" 
                                            {{ old('driver_id', optional($route->assignments->where('status', 'active')->first())->driver_id) == $driver->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" 
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('description', $route->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                <template x-for="(stop, index) in stops" :key="stop.id || index">
                                    <div class="border rounded-lg p-4 relative">
                                        ...
                                        <input type="hidden" x-model="stop.id" :name="'stops[' + index + '][id]'" />
                                    </div>
                                </template>
                            </div>
                            @error('stops')
                        ...
                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Update Route') }}</x-primary-button>
    ...
    @push('scripts')
    <script>
        function routeForm(existingStops = []) {
            return {
                stops: existingStops.map(stop => ({
                    id: stop.id,
                    name: stop.name,
